...
(function($) {
    ...
    $(document).on('click','.edit_search .vi',function () {
      var duongdan=$(location).attr('href');
      var xac_dinh=duongdan.split('/')[duongdan.split('/').length -1];
      var hientai=$('.edit_search').attr('arg');
      if(hientai==='tim-kiem')
      {
        location.assign(duongdan.split('/')[0]+'/vi/tim-kiem?'+xac_dinh.split('?')[1]);
      }
      else {
        if(hientai==='search-key')
        {
          location.assign(duongdan.split('/')[0]+'/vi/tim-kiem?'+xac_dinh.split('?')[1]);
        }
      }
    })
    $(document).on('click','.edit_search .en',function () {
      var duongdan=$(location).attr('href');
      var xac_dinh=duongdan.split('/')[duongdan.split('/').length -1];
      var hientai=$('.edit_search').attr('arg');
      if(hientai==='tim-kiem')
      {
        location.assign(duongdan.split('/')[0]+'/en/search-key?'+xac_dinh.split('?')[1]);
      }
      else {
        if(hientai==='search-key')
        {
    ...
    $(document).on('click','.search_mobile .show-searchbox-edit',function () {
      if($('.search_mobile').hasClass('hien_thi'))
      {
        $('.search_mobile').removeClass('hien_thi');
      }
      else
      {
        $('.search_mobile').addClass('hien_thi');
      }
    })
		/*=============================================*/
		/*------------ [_Portfolio_Filter] ------------*/
	...
	$('.section-news .news-wrapper .news-thumb').click(function() {
		window.location = $(this).find('a').attr('href');
		return false;
	})

	$('.images-section .case-study-item .case-study-thumb').click(function() {
		window.location = $(this).find('a').attr('href');
		return false;
	})

	$('.service-box-new ').click(function() {
		window.location = $(this).find('a').attr('href');
		return false;
	})
}(jQuery));
