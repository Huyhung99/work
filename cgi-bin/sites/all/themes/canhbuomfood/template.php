...
{
    $form['account']['login']['mail']['#attributes']['class']=array('form-control mb-10');
    $form['customer_profile_billing']['addressbook']['#attributes']['class']=array('form-control');
...
function minhhienstore2_form_views_exposed_form_alter(&$form, &$form_state, $form_id)
{
    if($form['#id'] == 'views-exposed-form-taxonomy-term-page-search-product'){
        ...
        $form['title']['#attributes']['placeholder'] = t('Tên sản phẩm');
        $form['field_product_category_tid']['#attributes']['class'] = array('form-control');
        $form['field_product_category_tid']['#options']['All']=t('-- Chọn loại sản phẩm --');
    }
