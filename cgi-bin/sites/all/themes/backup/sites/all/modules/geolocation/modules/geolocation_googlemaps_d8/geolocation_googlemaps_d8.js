...
(function ($) {
  ...
  Drupal.behaviors.geolocationGooglemaps = {
    attach: function(context, settings) {
      ...
      /**
       * Process Drupal.settings for every Google map present on the current page.
       */
      $.each(Drupal.settings.geolocation.defaults, function(canvasId, mapDefaults) {
       ...
       */
      $('.geolocation-map-canvas .geocode-controlls-wrapper .submit').live('click', function(event) {
       ...
       */
      $('.geolocation-map-canvas .geocode-controlls-wrapper .input').live('keypress', function(event){
        if (event.which == 13) {
          var $mapCanvas = $(this).closest('.geolocation-map-canvas');
          event.preventDefault();
          Drupal.geolocation.codeAddress($mapCanvas);
       ...
       * Click event handler to the 'Get location' button.
       */
      $('.geolocation-map-canvas .geocode-controlls-wrapper .clear').live('click', function(event) {
        var $mapCanvas = $(this).closest('.geolocation-map-canvas');
        event.preventDefault();
