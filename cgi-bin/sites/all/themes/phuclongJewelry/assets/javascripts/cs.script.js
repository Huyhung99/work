  ...
  Drupal.behaviors.mymodule = {
    attach: function (context, settings) {
      ...
      $('.add-to-cart').click(function (){
        $(this).parent().find('.hide.add-cart-to-form form').submit();
      });
      $(document).on('click', 'add-to-cart', function (e) {
        e.preventDefault();
        $(this).parent().parent().find('.hide.add-cart-to-form form').submit();
      })
      $('.qty-up').on('click',function (e){
        var value = $('.quantity-input-product').val();
        $(this).parent().siblings('input.quantity-input-product').val(parseInt(value)+1)
      })
      $('.qty-down ').on('click',function (e){
        var value = $('.quantity-input-product').val();
        $(this).parent().siblings('input.quantity-input-product').val(parseInt(value)-1)
        if (value <=1){
          $(this).parent().siblings('input.quantity-input-product').val(parseInt(1))
        }
      })
      ...
      $('.cart-items .quantity').find('input').addClass('form-control');
    }
  }
}(jQuery));
