...
(function($) {
    ...
    $(document).ready(function() {
        ...
        $(document).on('click','.wpcf7-submit',function () {
          $('.thong-tin').html('<a href="#" class="buttonload"><i class="fa fa-refresh fa-spin"></i></a><p class="text-center">Đang kiểm tra</p>')
          var ho_ten=$('.lgx-registration-form .ho_ten').val();
          var email=$('.lgx-registration-form .email').val();
          var so_dien_thoai=$('.lgx-registration-form .so_dien_thoai').val();
          var dt_ng_gt=$('.lgx-registration-form .dt_ng_gt').val();
          if(ho_ten==='' || email==='' || so_dien_thoai==='')
          {
            setTimeout(function () {
              $('.bat_tat').removeClass('d-none');
              $('.thong-tin').html('<h3 class="text-center">Nhập đầy đủ thông tin của những mục có dấu *</h3>');
            },1000);
          }
          else
          {
            $.ajax({
              url: 'https://luuhaiau.edu.vn/thao-tac-thong-tin',
              data: {
                ho_ten:ho_ten,
                email:email,
              ...
              success: function(data){
                ...
                setTimeout(function () {
                  ...
                  if(data.thong_bao1!=='')
                  {
                    $('.thong-tin').html('<h3 class="text-center">'+data.thong_bao1+'</h3>')
                  }
                  else
                  {
                    $('.thong-tin').html('<h3 class="text-center">Không phản hồi</h3>')
                  }
                },1000);
              },
